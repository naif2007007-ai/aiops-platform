...
ASSIGN_GROUPS = ["Network-Ops","Server-Ops","Storage-Ops",
                 "Security-Ops","Application-Ops","Field-Services"]
...
def generate_tickets(assets, alarms):
    ...
    for idx,(_, alarm) in enumerate(ta.iterrows()):
        ...
        # days since last change — failing assets have more recent changes
        if wf:
            days_since_change = int(rng.integers(1, 30))
        else:
            days_since_change = int(rng.integers(7, 180))
